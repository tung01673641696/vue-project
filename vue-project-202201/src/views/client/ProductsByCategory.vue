  ...
  <div>
    <div class="main__content">
      <div class="breadcrumb">
        <span class="breadcrumb--link__previous">Danh mục / </span>
        <span class="breadcrumb--link__current">{{ category_name }}</span>
      </div>
      <div class="main__content--page">
        ...
        <div class="products__list">
          ...
          <div v-if="!isHasProduct" class="errorNoProduct">
            <div class="">Không có sản phẩm <i class="far fa-frown" /></div>
            <router-link to="/">
...
import { API_PRODUCTS_BY_CATEGORY } from "@/common/constants/api.constants";
...
export default {
  ...
  created() {
    ...
    this.paramsCategory = this.$route.params.id;
  ...
  methods: {
    ...
    updateListItem() {
      this.listItem = this.products.slice(
        this.currentPage * this.pageSize,
        this.currentPage * this.pageSize + this.pageSize
      );
      if (this.listItem.length == 0 && this.currentPage > 0) {
    ...
    getList(page, price = this.maxPrice) {
      const url = `${API_PRODUCTS_BY_CATEGORY}?page=${page}&keyword=${this.$route.params.id}&keyword_price=${price}`;
      console.log(url);
      getProducts(url)
        .then((response) => {
