<template>
  <div class="main__content">
    <div class="breadcrumb">
      <span class="breadcrumb--link__previous">Tìm Kiếm / </span>
      <span class="breadcrumb--link__current">{{ keyword }}</span>
    </div>
...
export default {
  ...
  methods: {
    ...
    getList(page, price = this.maxPrice) {
      const url = `${API_SEARCH_PRODUCT}?page=${page}&keyword=${this.$route.query.key}&keyword_price=${price}`;
      getProducts(url)
        .then((response) => {
  ...
  // watch: {
  //   $route(to) {
  //     this.keyword = to.query.key;
  //     this.getProductByName(this.keyword);
  //   },
  // },
};
</script>
